...
class ArrowView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let vector = calculateVPoint(from: origin, to: targetPoint)
    ...
    // Triangle
    // A: endPoint
    // B: endPoint - point + pointClockwise
    // C: endPoint - point + pointAntiClockwise
    private func drawTriangle(with vector: CGPoint, from endPoint: CGPoint) {
        let path = UIBezierPath()
        let b = CGPoint(x: endPoint.x - vector.x + rotatePointBy90(vector, clockwise: true).x,
                        y: endPoint.y - vector.y + rotatePointBy90(vector, clockwise: true).y)
        let c = CGPoint(x: endPoint.x - vector.x + rotatePointBy90(vector, clockwise: false).x,
                        y: endPoint.y - vector.y + rotatePointBy90(vector, clockwise: false).y)
